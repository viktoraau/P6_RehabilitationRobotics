  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  for (auto & joint : joints_) {
    ...
    // MD dcBusVoltage is reported in volts and can be published directly.
    if (joint.has_voltage_state) {
  ...
  if (use_pds_ && power_stage_state_.exported) {
...
hardware_interface::return_type MABSystemHardware::read_power_stage_telemetry()
{
  ...
  power_stage_state_.bus_voltage = static_cast<double>(bus_voltage_mv) / 1000.0;
